...
function action_status()
    ...
    local data = {
        temp = util.trim(sys.exec("cat $(uci -q get fan_ctrl.settings.temp_sensor || echo '/sys/class/thermal/thermal_zone0/temp') | awk '{printf \"%.1f\", $1/1000}'")),
        pwm = util.trim(sys.exec("cat $(uci -q get fan_ctrl.settings.pwm_ctrl || echo '/sys/class/hwmon/hwmon0/pwm1') 2>/dev/null")),
